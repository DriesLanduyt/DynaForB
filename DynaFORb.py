...
#        self.leaf_fall = np.sum(np.array([Trees["dd_leaf_fall"][s] for s in self.species])*np.array(t_comp))
    ...
    def Dynamics(self,driver_dynamics):
        ...
        #light transmittance and LAI dynamics follow a generalised logistic fucntion, following Perot et al. (2019) using mid of spring and fall (inflection points) and speed of LAI change (cT) as parameters (see also Niinemets et al., 2005 and Xie et al., 2005 for more complex representations)
        #TO DO: calculate mid of spring (MOS) and mid of fall (MOF) based on temperature data
...
class Vegetation():
    ...
    def Dynamics(self,microclimate_dynamics):
        ...
        #based on degreedays -> automatic switch between two: if all(np.isin(self.start,np.arange(1,13))): go to month version below
        #start_indices = [np.argmax(microclimate_dynamics["degreedays"]>t) for t in self.start] #argmax looks at first TRUE value, will become DEPRECATED
        #max1_indices = [np.argmax(microclimate_dynamics["degreedays"]>t) for t in self.max1]
        #max2_indices = [np.argmax(microclimate_dynamics["degreedays"]>t) for t in self.max2]
        #stop_indices = [np.argmax(microclimate_dynamics["degreedays"]>t) for t in self.stop]
        
        #convert parameters to dates
        start_indices = [self.dynamics.index[self.dynamics.dates.dt.month==T].min() - 15 for T in self.max1] #assuming emergence 15 days prior to flowering
        ...
        stop_indices = [self.dynamics.index[self.dynamics.dates.dt.month==T].min() for T in self.max2] #assuming senescence emmediately after flowering
    ...
    def Grow(self,growth_input):
        ...
        pnmax_leafN_fac = np.minimum(self.LNC/(self.Nref*constants["C_biomass_ratio"]*1000),1)  #self.LNC in mg/g DM, self.Nref in mg/gC
        pnmax_adj = self.pnmax*pnmax_T_fac*pnmax_leafN_fac      
        
        #alpha
        alpha_leafN_fac = np.minimum(0.5+0.5*(self.LNC/(self.Nref*constants["C_biomass_ratio"]*1000)),1)
        ...
        rm_leafN_fac = self.LNC/(self.Nref*constants["C_biomass_ratio"]*1000) #same constants as pnmax adjustment
